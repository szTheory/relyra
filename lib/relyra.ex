defmodule Relyra do
  ...
  def start_login(connection, relay_context, opts \\ []) do
    with {:ok, request_fields} <- Relyra.Protocol.AuthnRequest.build(connection, relay_context, opts),
         request_id <- Map.fetch!(request_fields, :id),
         authn_request_xml <- AuthnRequest.to_xml(request_fields),
         {:ok, relay_state} <-
           Relyra.Security.RelayState.issue(Map.put(relay_context, :request_id, request_id), opts),
         {:ok, redirect_params} <- Relyra.Protocol.Binding.encode_redirect(authn_request_xml, relay_state) do
      {:ok,
       %{
