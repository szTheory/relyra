...
- [x] **SIGV-04** — Metadata-root signatures (`EntityDescriptor`/`EntitiesDescriptor`) are cryptographically verified using the same primitive (signature math, not fingerprint-pinning alone), preserving operator-pinned trust (`TrustAnchor`) as defense-in-depth.
