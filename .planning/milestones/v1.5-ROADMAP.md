...
**Phases:** 41-46 (6 phases)
**Total Plans:** 18
**Requirements:** 15/15 complete

## Overview

Close the gap between code shipped to git (v1.3/v1.4 features) and what adopters can actually see on Hex — publish `1.4.0`, make the "every login explains itself" promise visible via stepwise login-trace LiveView, and close residual DX and warning-level tech-debt so a new adopter installs in 15 minutes. After v1.5, future scope is demand-gated.

---

# Roadmap: Relyra (archived at milestone close)
...
- TD-02 (test_support prod exclusion) must land before Phase 44 publish — tarball correctness takes precedence over phase-ordering convenience.
- Phase 42 (trace) sequenced before Phase 43 (publish prep) so trace UI ships in the `v1.4.0` Hex tarball by construction.
- Single Hex jump 1.2.0 → 1.4.0; CHANGELOG retains `[1.3.0]` section without intermediate Hex release.
- TRACE LiveView reuses telemetry + audit ledger only — no new schemas (audit co-commit invariant preserved).

**Issues Resolved:**

- Hex publishing lag (mix.exs at 1.2.0 while git had v1.4 features).
- Release-please stall (stale PR #5, 115 unpushed commits).
- v1.3 audit warnings WR-01/02/03/04, WR-ENC-ATTR, Phase 40 formatting drift.
- No stepwise login-trace UI for "every login explains itself" brand promise.
- README/installer DX friction (no above-fold snippet, manual router wiring).

**Issues Deferred:**

- Phase 15 VERIFICATION (human_needed) — acknowledged in STATE.md Deferred Items.
- v1.4 follow-up review items WR-01/WR-02 in drift test — non-blocking.
- AUTHN-POST-01, KMS-01, SIGNED-META-01 — demand-gated per assessment.

**Technical Debt Incurred:**

None new. v1.5 was explicitly a tech-debt and publish closure milestone.
