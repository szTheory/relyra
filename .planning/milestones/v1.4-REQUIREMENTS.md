...
- [ ] **SLO-01**: Full SLO (Single Logout) round-trip (SP-initiated + IdP-initiated) with `SessionIndex` correlation. This involves extending the `Relyra.SessionAdapter` behavior (`index_session/4` + `terminate_by_session_index/4`), handling strict signature verification of `LogoutRequest`/`LogoutResponse`, and replay protection.
