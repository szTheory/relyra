...
- [x] **ENC-01**: SP can decrypt `EncryptedAssertion` using RSA-OAEP key transport and AES-GCM content encryption; decrypted bytes pass through `PureBeam.parse_safely/2` (hardened saxy seam) and `Signature.do_verify/4` (XMLDSig verification) before any identity fields are read; all decryption failures return opaque `:decryption_failed` atom regardless of failure mode. (`EncryptedAttribute` was research scope only — not shipped in v1.3.)
