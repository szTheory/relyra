...
Base image: `python:3.12-slim` (linux/arm64), pulled 2026-05-23.
...
4. **Implementation under test** — `Relyra.Security.XML.C14N` via `PureBeam.parse_safely → select_signed_node
   → canonicalize` produces the **byte-identical 887 bytes** (verified in `corpus_security_test.exs`,
