defmodule Relyra.LiveAdminMetadataTest do
  ...
  test "mount/3 initializes assigns" do
    ...
    assert {:ok, new_socket} =
             ConnectionMetadataLive.mount(%{"connection_id" => "conn_meta_test"}, %{}, socket)
  ...
  test "handle_params/3 updates mode to xml" do
    ...
    assert {:noreply, new_socket} =
             ConnectionMetadataLive.handle_params(%{"mode" => "xml"}, "uri", socket)
  ...
  test "handle_params/3 updates mode to url" do
    ...
    assert {:noreply, new_socket} =
             ConnectionMetadataLive.handle_params(%{"mode" => "url"}, "uri", socket)
  ...
  test "render/1 displays xml mode" do
    assigns = %{
      ...
      streams: %{
        metadata_revisions: %Phoenix.LiveView.LiveStream{
    ...
    html =
      Phoenix.HTML.Safe.to_iodata(ConnectionMetadataLive.render(assigns)) |> IO.iodata_to_binary()
  ...
  test "handle_event refresh_metadata triggers start_async and sets loading state" do
    ...
    assert {:noreply, new_socket} =
             ConnectionMetadataLive.handle_event("refresh_metadata", %{}, socket)
  ...
  test "handle_async :metadata_refresh success updates status and flashes info" do
    ...
    assert {:noreply, new_socket} =
             ConnectionMetadataLive.handle_async(:metadata_refresh, {:ok, {:ok, %{}}}, socket)
  ...
  test "handle_async :metadata_refresh error updates status and flashes error" do
    ...
    assert {:noreply, new_socket} =
             ConnectionMetadataLive.handle_async(
               :metadata_refresh,
               {:ok,
                {:error, %Relyra.Error{type: :metadata_refresh_failed, message: "Refresh failed"}}},
               socket
             )
  ...
  test "handle_async :metadata_refresh exit updates status and flashes error" do
    ...
    assert {:noreply, new_socket} =
             ConnectionMetadataLive.handle_async(:metadata_refresh, {:exit, :timeout}, socket)
