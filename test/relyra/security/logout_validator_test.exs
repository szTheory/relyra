defmodule Relyra.Security.LogoutValidatorTest do
  ...
  describe "validate_logout_request/3 (redirect binding)" do
    test "success: strictly verifies signature, inflates payload, checks replay, returns typed struct",
         %{opts: opts} do
      ...
      assert {:ok, %{id: _} = req} =
               LogoutValidator.validate_logout_request(
                 query,
                 @connection,
                 opts ++ [binding: :redirect]
               )
    ...
    test "failure: issuer mismatch", %{opts: opts} do
      ...
      assert {:error, %Error{type: :issuer_mismatch}} =
               LogoutValidator.validate_logout_request(
                 query,
                 @connection,
                 opts ++ [binding: :redirect]
  ...
  describe "validate_logout_response/3 (redirect binding)" do
    test "success: returns typed LogoutResponse struct", %{opts: opts} do
      ...
      assert {:ok, %{id: _} = res} =
               LogoutValidator.validate_logout_response(
                 query,
                 @connection,
                 opts ++ [binding: :redirect]
               )
    ...
    test "failure: invalid status code returns error", %{opts: opts} do
      ...
      assert {:error, %Error{type: :unsupported_status}} =
               LogoutValidator.validate_logout_response(
                 query,
                 @connection,
                 opts ++ [binding: :redirect]
  ...
  describe "validate_logout_request/3 (post binding)" do
    test "failure: missing signature rejects immediately before replay", %{opts: opts} do
      ...
      assert {:error, %Error{type: :missing_signature}} =
               LogoutValidator.validate_logout_request(xml, @connection, opts ++ [binding: :post])
    end
  end
